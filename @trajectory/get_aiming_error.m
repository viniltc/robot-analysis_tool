...
%       'regr': linear regression over all samples in the interval
%       'final': only use first and last point in the interval
% (C) V. Sanguineti, 2008
...
target_traj = traj.xyT(end,:)- traj.pos(traj.rind,:);
...
switch (type)
    ...
    case 'final',
       rt1 = diff(early_traj(points,1));
       rt2 = diff(early_traj(points,2));
end   
...
actual_dir=atan2(target_traj(2),target_traj(1));
