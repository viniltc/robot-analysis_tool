...
for ha=1:traj.nhands
    switch traj.trsize
        case 3, % in 3D trajectories, display [YZ] projection
           ...
           corr = xcorr(traj.vel(:,3*(ha-1)+2),traj.vel(:,3*(ha-1)+3));
        otherwise, 
            speed = sqrt(traj.vel(:,2*(ha-1)+1).^2+traj.vel(:,2*(ha-1)+2).^2);
           corr = xcorr(traj.vel(:,2*(ha-1)+2),traj.vel(:,2*(ha-1)+3));
    end
    if traj.nhands ==1
      line(traj.time(traj.interval{ha})-traj.time(traj.interval{ha}(1)),speed(traj.interval{ha}),'col',col,'lines','-') 
     % line(traj.time(traj.interval{ha})-traj.time(traj.interval{ha}(1)),corr(traj.interval{ha}),'col',col,'lines','*')
    
    else
       line(traj.time(interval)-traj.time(interval(1)),speed(interval),'col',cols(ha,:),'lines','-') 
     % line(traj.time(interval)-traj.time(interval(1)),xcorr(interval),'col',cols(ha,:),'lines','*')
