function plot_vpdistances(tset,phaseno,mode,par)
bwfigs = 0;
if bwfigs
    col_avg='k';
    col_fig='k';
...
scalebar.xdim=0.20;
scalebar.ydim=0.10;
spdstrval=['VPdistance - ', num2str(phaseno)];
...
if length(th)>1
    for i = 1:length(th)
            ...
            ylabel(['VP distance [', munit,']'], 'FontSize',15);
            set(gca,'vis','off');
    end
else
    hva=gca;
...
for i = 1:length(th)
           ...
           if mean(isforce(tset.traj{trial}))>0.5 % force is on
               plot_vpdistance(tset.traj{trial},col_for);
           else
               plot_vpdistance(tset.traj{trial},col_nul);
           end
           hold on %%
            %plot(traj.time(traj.interval{par}(i1))*[1 1],[0 0.1],'k--')%%%%%%
            %plot(traj.time(traj.interval{par}(i2))*[1 1],[0 0.1],'k--') %%
end   
end
ylim([0 0.1])